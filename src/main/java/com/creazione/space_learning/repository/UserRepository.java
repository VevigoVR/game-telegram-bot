...
public interface UserRepository extends JpaRepository<UserEntity, Long> {
    ...
    @Query("SELECT DISTINCT u FROM UserEntity u LEFT JOIN FETCH u.buildings WHERE u.telegramId = :telegramId")
    Optional<UserEntity> findByTelegramIdWithBuildings(@Param("telegramId") Long telegramId);
    @Query("SELECT DISTINCT u FROM UserEntity u LEFT JOIN FETCH u.resources WHERE u.telegramId = :telegramId")
    Optional<UserEntity> findByTelegramIdWithResources(@Param("telegramId") Long telegramId);
    ...
    Optional<UserEntity> findFullUserByTelegramId(@Param("telegramId") Long telegramId);

    // Исправленный count-метод (без FETCH)
    @Query("SELECT COUNT(DISTINCT u.id) FROM UserEntity u " +
    ...
    @Query("SELECT DISTINCT u FROM UserEntity u " +
            ...
            "WHERE b.lastUpdate >= :cutoff " +
            "ORDER BY u.id")
    Page<UserEntity> findUsersWithRecentBuildingUpdates(
    ...
    @Query("SELECT DISTINCT u FROM UserEntity u " +
            ...
            "LEFT JOIN FETCH u.notices " +
            "ORDER BY u.id")
    Page<UserEntity> findUsersWithResourcesAndRecentBuildingUpdates(
            Pageable pageable
    );
    ...
    @Query("SELECT DISTINCT u FROM UserEntity u " +
            "LEFT JOIN FETCH u.resources " +
            "WHERE u.id = :id")
    Optional<UserEntity> findUserWithResourcesById(Long id);
    ...
    @Query("SELECT u.telegramId FROM UserEntity u WHERE u.id = :id")
